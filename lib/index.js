...
Object.defineProperties(subject.loggers, {
	...
	"anonymous": { value: new LoggerProxy("anonymous"), writable: true, enumerable: true, configurable: false },
});
Object.defineProperties(subject.loggers["anonymous"], {
...
function colorizeLevels(colorMap, levelMapper){
	return subject.colorizeLevels.apply(subject, arguments);
}
function decolorizeLevels(colorMap, levelMapper){
	return subject.decolorizeLevels.apply(subject, arguments);
}
...
function createMiddleware(opts){
	...
	if (!opts.hasOwnProperty("handle")){
		if (!subject.handles.hasOwnProperty("middleware")){
	...
	//Create new Logger
	mwLogger = new LoggerProxy(opts["name"]);
...
function createRequestListener(opts){
	...
	if (!opts.hasOwnProperty("handle")){
		if (!subject.handles.hasOwnProperty("reqListen")){
			opts.handle = subject.addHandle({
				"name": "reqListen",
				"target": "console",
				"formatter": "combined",
	...
	//Create new Logger
	reqLogger = new LoggerProxy(opts["name"]);
...
function setUncaughtOpts(opts){
	for (key in opts){
		subject.handles["__exception__"][key] = opts[key];
	}
}

function exception(errorObj){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["error"].concat(Array.prototype.slice.call(arguments)));
}
function error(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["error"].concat(Array.prototype.slice.call(arguments)));
}
function warn(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["warn"].concat(Array.prototype.slice.call(arguments)));
}
function info(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["info"].concat(Array.prototype.slice.call(arguments)));
}
function verbose(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["verbose"].concat(Array.prototype.slice.call(arguments)));
}
function debug(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["debug"].concat(Array.prototype.slice.call(arguments)));
}
function silly(message){
	subject.loggers["anonymous"].log.apply(subject.loggers["anonymous"], ["silly"].concat(Array.prototype.slice.call(arguments)));
}
...
module.exports = {
	...
	"on": on,
	"setUncaughtOpts": setUncaughtOpts,
