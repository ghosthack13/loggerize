...
function createHTTPLogger(opts){
	...
	if (opts.hasOwnProperty("levelMapper")){
		if (!subject.levelMappings.hasOwnProperty(opts["levelMapper"])){
			let err = new TypeError("'" + opts["levelMapper"] + "' is not a valid level mapper. Use the `defineLevels` method to create it.");
			throw err;
		}
	}
	else {
	...
	opts["logOnRequest"] = opts["logOnRequest"] || false;
	opts["logOnResponse"] = opts["logOnResponse"] || true;
