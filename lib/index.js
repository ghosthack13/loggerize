...
*	@param {string} mapper - Name of a library-defined or user-defined levelMapper
*	@throws {TypeError} Argument mapper must be of type string
*/
...
*	@param {(string|number)} level - Desired default severity level
*	@throws {Error} Throws if level is not compatible with the selected levelMapper
*	@returns {string} level
*/
function setLevel(level){
	subject.loggers["_anonymous"].setLevel(level);
...
*	@returns {object} An instance of the standard application logger
*
*/
...
*	@returns {object} An instance of a standard HTTP logger
*
*/
...
*	@param {object.<string,string|number>} tokens - An object that maps token names to values
*	@throws {Error} Throws if there is an attempt to overwrite a predefined library token
*/
function addTokens(){
	return subject.addTokens.apply(subject, arguments);
}

/**	Add a handle configuration to deal with uncaught exceptions
...
*	@param {function} func - The definition of the target
*	@throws {TypeError} Argument name must be of type object/function
...
*	@param {function} func - The definition of the filter function
...
*	@param {function} func - The definition of the transformer function
*	@throws {TypeError} Argument name must be of type object/function
...
/**	@param {string} eventName - Name of the event that is being listened
*	@param {logEventCallback} callback - Name of the event that is being listened
*/
function on(){
...
//================================================================================
// Create Predetermined Loggers
//================================================================================

//! Create root and anonymous loggers
subject.loggers["root"] = createLogger({
	"name": "root",
	"levelMapper": "npm",
	"hasHandles": false,
});
subject.loggers["_anonymous"] = createLogger({
	"name": "_anonymous",
	"level": "debug",
	"levelMapper": "npm",
	"propogate": false,
	"handle": {
		"name": "_anonymous" + Date.now(), 
		"target": "console",
		"formatter": {
			"name": "_anonymous" + Date.now(), 
...
//! Make root and anonymous loggers non-configurable
Object.defineProperties(subject.loggers, {
	"root": 	 { writable: true, enumerable: true, configurable: false },
	"_anonymous": { writable: true, enumerable: true, configurable: false },
});


//================================================================================
// API for the Anonymous Logger
//================================================================================

function setConsoleTarget(){
	
	//Get name of handle attached to the anonymous handle
	let oldHandleName = subject.loggers["_anonymous"].handles[0];
	subject.removeHandle(oldHandleName);
	
	//Add new anonymous handle with console target
	let newHandle = {
		"name": "_anonymous" + Date.now(), 
		"target": "console"
	};
	subject.loggers["_anonymous"].attachHandles(newHandle);
}

function setFileTarget(path){
	
	//Get name of handle attached to the anonymous handle
	let oldHandleName = subject.loggers["_anonymous"].handles[0];
	subject.removeHandle(oldHandleName);
	
	//Add new anonymous handle with file target
	let newHandle = {
		"name": "_anonymous" + Date.now(), 
		"path": path,
	};
...
function setFormat(formatStr){
	...
	let handleName = subject.loggers["_anonymous"].handles[0];
	let oldFormatterName = subject.handles[handleName]["formatter"];
	subject.removeFormatter(oldFormatterName);
	
	//Add new anonymous formatter
	let newFormatterName = "_anonymous" + Date.now();
	let newFormatter = {
		"name": newFormatterName,
		"format": formatStr
	};
	subject.addFormatter(newFormatter);
	
	//Update formatter in the handle
	subject.handles[handleName]["formatter"] = newFormatterName;
}
