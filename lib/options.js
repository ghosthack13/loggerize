...
var validFormatterOpts = [
	"name", "transformer", "colorMap", "background", "font",
	"defaultSubstitution", "format", "json", "fields",
	"uuid", "datetime", "message", "level",
];
var validHandleOpts = [
...
var validHTTP = [
	...
	//Express
	"app", "baseUrl", "body", "cookies", "fresh", "hostname", "ip", "ips", 
	"method", "originalUrl", "params", "path", "protocol", "query", "route", 
	"secure", "signedCookies", "stale", "subdomains", "xhr ", 

	//Standard Request Fields
	"accept", "acceptEncoding", "acceptLanguage",
	"connection", "cookie", "host", "userAgent", 
	"contentLength", "contentType", "forwarded", 
	"pragma", "origin", "referer", "upgrade", 

	//Common Non-Standard Request Fields
	"dnt", "xRequestedWith", "xForwardedFor", 
	"xForwardedHost", "xForwardedProto", 

	//Standard Response Fields
	"xPoweredBy", "server", "pragma", 
	"location", "lastModified", "expires", 
	"connection", "contentLength", 
	"contentType", "contentEncoding", 
	"contentLanguage", "accessControlAllowOrigin", 

	//Common Non-Standard Request Fields
	"status", 
	
	
	//Accept the following as workable spelling mistakes
	"referrer",
];

var predefinedTargets = [
	null, "console", "file", "rotatingFile", "rest"
];
...
function validateHandleOpts(opts){
	...
	//validate directory, fileName, maxSize, maxFiles, url, port, method, 
	...
	if (opts.hasOwnProperty("target")){
		...
		if (opts["target"] == "rest"){
			if (!opts.hasOwnProperty("url")){
				let err = Error("the REST target requires a url to be defined");
				throw err;
			}
		}
	}
	...
	if (opts.hasOwnProperty("rotationType")){
		if (!rotationTypes.includes(opts.rotationType)){
	...
	if (opts.hasOwnProperty("interval")){
		...
		let validIntervals = ["year", "month", "week", "day", "hour", "minute", "second"];
		let dayIndex = {
	...
	if (opts.hasOwnProperty("maxFiles")){
		if(typeof(opts["maxFiles"]) == "string"){
			opts["maxFiles"] = Number(opts["maxFiles"]);
		}
	...
	if (opts.hasOwnProperty("filter")){
		...
		if (!Array.isArray(opts["filter"]) && typeof(opts["filter"]) != "function"){
...
function setHandleDefaults(opts){
	...
	if (!opts.hasOwnProperty("level")){
		...
		if (levelMap.hasOwnProperty("debug")){
			opts["level"] = "debug";
		}
		else{
	...
	if (opts["target"] == "file" || opts["target"] == "rotatingFile"){
		...
		if (!opts.hasOwnProperty("directory")){
			if (opts.hasOwnProperty("path")){
				...
				opts["directory"] = "." + path.sep + "logs" + path.sep;
			}
	...
	if (opts["target"] == "rest"){
		...
		opts["port"] 	= urlObj["port"] || opts["port"] || "80";
		opts["headers"] = {"User-Agent": "Remote Logger"};
		...
		Object.defineProperty(opts, "connection", {
			...
			enumerable: false, 
			configurable: false, 
			value: (urlObj["protocol"].indexOf("https") != -1) ? https : http
		});
	}
...
function configure(options){
	...
	if (options.hasOwnProperty("level")){
		this.validateOptions({"level": options.level});
		this.DEFAULT_LEVEL = options.level;
	}
	
	if (options.hasOwnProperty("target")){
		this.validateOptions({"target": options.target});
		this.DEFAULT_TARGET = options.target;
	}
	
	if (options.hasOwnProperty("fileExtension")){
		this.validateOptions({"fileExtension": options.fileExtension});
		this.DEFAULT_EXTENSION = options.fileExtension;
	}
	
	
	if (options.hasOwnProperty("formatters")){
		let formatterList = Object.keys(options["formatters"]);
		for (let i = 0; i < formatterList.length; ++i){
			addFormatter(formatterList[i]);
		}
	}
	
	if (options.hasOwnProperty("handles")){
		let handleList = Object.keys(options["handles"]);
		for (let i = 0; i < handleList.length; ++i){
			let handleName = handleList[i];
			if (handleName != "anonymous"){
				addHandler({handleName : options["handles"][handleName]});
			}
		}
	}
	else if (Object.keys(this.handles).length == 1){
		this.addHandler({"anonymous": options});
		return;
	}
	else if (this.handles.hasOwnProperty("anonymous")){
		this.setHandleOpts(options, "anonymous");
		return;
	}
...
function configureFromFile(targetPath){
	fs.readFile(path.normalize(targetPath), {encoding: 'utf-8'}, function (err, data) {
		if (err) throw err;
