...
// Library name plus the version in SemVer format, i.e. <Major>.<Minor>.<Patch>
var DEFAULT_USER_AGENT = "loggerize" + path.sep + require(
...
var validHandleOpts = [
	...
	"maxFiles", "maxSize",
	"url", "port", "method", "headers", "allowInsecure", "timeout",
	"emitEvents", 
];
var validIntervals = [
...
function cloneConfig(opts){
	...
	//initially make a shallow clone
	let clonedObj = JSON.parse(JSON.stringify(opts));
	
	//Iterate object and bind functions to cloned object
	for (let prop in opts){
		if (typeof(opts[prop]) == "function" && opts.hasOwnProperty(prop)){
			clonedObj[prop] = opts[prop].bind({});
		}
	}
...
function validateHandleOpts(opts){
	...
	if (predefinedTargets.indexOf(opts["target"]) != -1){
		for (let key in opts){
			if (opts.hasOwnProperty(key)){
				...
				if (!validHandleOpts.includes(key)){
	...
	if (opts.hasOwnProperty("emitEvents")){
		...
		else if (opts["emitEvents"] == "false"){
			opts["emitEvents"] = false;
		}
		
		if (typeof(opts["emitEvents"]) !== "boolean"){
			let err = new TypeError("emitEvents property expects value of type 'boolean'");
			throw err;
		}
	...
	if (opts.hasOwnProperty("directory")){
		...
		//Transform to full path if supplied relative directory
		if (opts["directory"].charAt(0) == "."){
			opts["directory"] = path.join(process.cwd(), opts["directory"] + path.sep);
		}
		
		//Add path separator to end of directory
	...
	if (opts.hasOwnProperty("target")){
		...
		if (opts["target"] == "http"){
			...
			if (/^https/i.test(opts["url"]) == false && !opts.hasOwnProperty("allowInsecure")){
				let err = Error("Designated URL is not an HTTPS resource. Use the option 'allowInsecure' to permit unencrypted HTTP requests");
				throw err;
			}
			...
			if (opts.hasOwnProperty("timeout")){
				...
				if (typeof(opts["timeout"]) != "number"){
					let err = Error("'timeout' must be of type 'number'");
					throw err;
				}
			}
...
function validateFormatterOpts(opts){
	...
	for (let key in opts){
		...
		//Reconstruct user defined style to be interpreted in the backend
		if (!nonTokens.hasOwnProperty(key) && opts[key].hasOwnProperty("style")){
			if (typeof(opts[key]["style"]) == "string"){
				opts[key]["style"] = opts[key]["style"].replace(/grey|gray/i, "blackbright");
			}
			else if (Array.isArray(opts[key]["style"])){
				opts[key]["style"] = opts[key]["style"].map(function(val){
					if (/grey|gray/i.test(val)){
						return "blackbright";
					}
					else{
			...
			Object.assign(opts[key], styler.parseStyle(opts[key]["style"]));
			delete opts[key]["style"];
		}
	}
	...
	if (opts.hasOwnProperty("format")){
		...
		if (!opts.hasOwnProperty("json") || opts["json"] == false){
			let err = Error("formatters must have at least the format property present or the json property set as 'true'");
			throw err;
		}
	}
	...
	if (opts.hasOwnProperty("json")){
		...
		//Convert string of boolean name to boolean type
		if (opts["json"] === "true"){
			opts["json"] = true;
		}
		...
		//validate that type if boolean
		if (typeof(opts["json"]) !== "boolean"){
			let err = new TypeError("The json property strictly accepts boolean values");
			throw err;
		}
		
		if (opts.hasOwnProperty("fields")){
			if (!Array.isArray(opts["fields"])){
				let err = new TypeError("The fields property must be of type 'string[]'");
				throw err;
			}
		}
	}
...
function setFormatterOpts(opts, formatterNames){
	...
	//Recursively call setFormatterOpts if given a list of formatters
...
function setHandleDefaults(opts){
	...
	if (!opts.hasOwnProperty("level")){
		...
		if (opts["levelMapper"] == this.levelMapper){
			opts["level"] = this.level;
		}
		else{ 
			let minMaxTuple = this.getMinMaxSeverity(opts["levelMapper"]);
			opts["level"] = this.getLevelName(minMaxTuple[0], opts["levelMapper"]);
		}
	}
	...
	if (!opts.hasOwnProperty("formatter")){
		opts["formatter"] = (opts["levelMapper"] == "http") ? "common" : "default";
	}
	...
	if (opts["target"] == "http"){
		...
		if (!opts.hasOwnProperty("allowInsecure")){
		...
		else if (typeof(opts["headers"]["User-Agent"]) == "undefined"){
