...
function createLevelMap(mapper, levelsObj, orderOfSeverity){
	...
	if (this.loggers.hasOwnProperty("root")){
		let loggerPrototype = this.loggers["root"].__proto__;
		for (let level in levelsObj){
			/*eslint-disable-next-line no-unused-vars */
			loggerPrototype[level] = function(message){
				loggerPrototype.log.apply(this, [level].concat(Array.prototype.slice.call(arguments)));
			};
		}
	}
}
...
function setLevel(level){
	...
	level = (typeof(level) == "number") ? this.getLevelName(level) : level;
