...
function testHandlers(){
	...
	logger.addFormatter({"name": "file_fmt", "format": "%{level} %{datetime} %{message}"});
...
function test_AddFormatter(){
	...
	// logger.addFormatter({"file_fmt": {"format": "%{level} %{datetime} %{message}"}});
...
function test_TokenParser(){
	...
	Library.addFormatter({
		...
		"format": "%%{level} %{datetime} %{level} %{uuid} '%{message}'"
	});
...
function test_LogFormats(){
	...
	logger.addFormatter([
		...
		//Extended Log Format		
		{
			...
			"format": "%{datetime} %{method} %{originalUrl}"
		},		
		...
		//Combined Log Format
		{
			...
			"format": '%{ip} - [%{datetime}] "%{method} %{originalUrl} %{protocol}" %{status} %{contentLenght} %{referer} %{userAgent}'
		},
		...
		//Common Log Format
		{
			...
			"format": '%{ip} - [%{datetime}] "%{method} %{originalUrl} %{protocol}" %{status} %{contentLenght}'
		}
...
function profilerRate(){
	...
	Library.addHandle({
		"name": "myHandle",
		// "formatter": "simple",
		"formatter": {"name": "myFormatter", "format": "%{datetime} %{level} %{uuid} %{message}"},
		// "target": "console",
		"target": "null",
	});
