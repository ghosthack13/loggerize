...
LoggerProxy.prototype.log = function(){
	...
	for (let i = tree.length; i >= 0; --i){
		...
		//When HTTP logging check for request/response go-ahead
		if (logRecord["_isHTTPRecord"]
		&& ((this.logOnResponse == false && logRecord.hasOwnProperty("statusCode"))
		|| (this.logOnRequest == false && !logRecord.hasOwnProperty("statusCode")))){
		...
		//Run level Filters
		if (!subject.filterLevel(logRecord, this)){
			//Emit filtered
			if (this.emitEvents === true){
				subject.emit("filtered", logRecord);
			}
