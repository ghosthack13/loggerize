...
LoggerProxy.prototype.setLevelMapper = function(levelMapper){
	...
	if (!subject.levelMappings.hasOwnProperty(levelMapper)){
		let err = new TypeError("'" + levelMapper + "' is not a valid level mapper. Use the `createLevelMap` method to create it.");
		throw err;
	}
...
LoggerProxy.prototype.on = function(eventName, listener){
	...
	subject.on(eventName, function(logRecord){
		if (this.emitEvents === true && self.name == logRecord["loggerName"]){
			listener(logRecord);
		}
	});
}
...
LoggerProxy.prototype.log = function(){
	...
	for (let i = tree.length; i >= 0; --i){
		...
		if (logRecord["_isHTTPRecord"] && !logRecord.hasOwnProperty("statusCode")){
			//Do not filter http request event (w/o response) since request will not have a status code/levelNum to check against
		}
