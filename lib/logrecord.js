...
function cloneRecord(logRecord){
	...
	if (logRecord["levelNum"]){
		record["levelNum"] = logRecord["levelNum"];
	}
	if (logRecord["uuid"]){
		record["uuid"] = logRecord["uuid"];
	}
	if (logRecord["loggerName"]){
		record["loggerName"] = logRecord["loggerName"];
	}
	...
	//Clone optional HTTP info
	if (record["user"]){
		record["user"] = logRecord["user"];
	}
	if (logRecord["statusCode"]){
