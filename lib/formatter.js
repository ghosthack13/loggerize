...
function format(logRecordRef, formatterName, levelMapper){
	
	// let dateObj = new Date(logRecordRef["DateObj"].getTime());
	// let logRecord = JSON.parse(JSON.stringify(logRecordRef));
	// logRecord["DateObj"] = dateObj;
	let logRecord = logRecordRef;
	...
	&& this.formatters[formatterName]["timestamp"].hasOwnProperty("pattern")){
		...
		let rfc3339 = logRecord["DateObj"].getDate()
			...
			+ " " + logRecord["DateObj"].toTimeString().slice(0, 17).replace(/GMT|UTC/gi, "");
