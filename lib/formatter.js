...
function format(logRecord, formatterName, levelMapper){
	...
	//Create datetime token from Javascript Date Object
	logRecord["datetime"] = logRecord["DateObj"].toLocaleString();
	if (this.formatters[formatterName].hasOwnProperty("datetime")){
		// console.log("timezone: ", this.formatters[formatterName]["datetime"]);
		//Formate datetime according to pattern
		if (this.formatters[formatterName]["datetime"].hasOwnProperty("pattern")){
			if (this.formatters[formatterName]["datetime"]["pattern"] == "ISO"){
				logRecord["datetime"] = logRecord["DateObj"].toISOString();
			}
			else{
				let timezone = this.formatters[formatterName]["datetime"]["timezone"];
				logRecord["datetime"] = parser.strptime(this.formatters[formatterName]["datetime"]["pattern"], timezone, logRecord["DateObj"]);
			}
		}
		
		//Style Datetime string
		logRecord["datetime"] = styler.ANSIStyler(logRecord["datetime"], this.formatters[formatterName]["datetime"]);
	}
...
function addTokens(tokens){
	...
	var immutableTokens = ["datetime", "level", "message", "uuid"];
...
function clearFormatters(){
	...
	//Set Formatters
	this.formatters = {
		"simple": {"format": "%{datetime} %{level} %{message}"},
		"common": { //Common Log Format. See https://httpd.apache.org/docs/1.3/logs.html
			"defaultSubstitution": "-",
			"datetime": {"pattern": "%d/%b/%Y:%H:%M:%S %z"},
			"format": '%{remoteIPv4} %{RFC1413identity} %{user} [%{datetime}] "%{method} %{url} %{protocol}/%{version}" %{statusCode} %{req.contentLength}'
		},
		"combined": { //Combined Log Format. See https://httpd.apache.org/docs/1.3/logs.html
			"defaultSubstitution": "-",
			"datetime": {"pattern": "%d/%b/%Y:%H:%M:%S %z"},
			"format": '%{remoteIPv4} %{RFC1413identity} %{user} [%{datetime}] "%{method} %{url} %{protocol}/%{version}" %{statusCode} %{res.contentLength} %{referer} %{userAgent}'
		}
	};		
