...
function substituteTokens(targetStr){
	
	let spaced = '\'' + targetStr.replace(/(%{.+?})/g, "' + $1 + '") + '\'';
	let substituted = spaced.replace(/%{(.+?)}/g, "logRecord['$1']");
...
function format(logRecord, formatterName, levelMapper){
	...
	let tokenCheck = null;
	let patt = /%{(.+?)}/g; //patt must not be dedined in loop else will run continuously
	...
	if (this.formatters["_levelStyle"][levelMapper]){ 
		...
		&& typeof(this.formatters[formatterName]["level"]["background"]) == "undefined")){
			// compiled = compiled.replace("logRecord['level']", styler.ANSIStyler("logRecord['level']", this.formatters["_levelStyle"][levelMapper][logRecord["level"]]));
...
function deprecated_format(logRecord, formatterName, levelMapper){
	...
	//Create temp format
	let tempFormat = this.formatters[formatterName]["_format"];
	...
	if (this.formatters["_levelStyle"][levelMapper]){ 
		...
		|| (typeof(this.formatters[formatterName]["level"]["font"]) == "undefined" 
		&& typeof(this.formatters[formatterName]["level"]["background"]) == "undefined")){
			tempFormat = tempFormat.replace("%{level}", styler.ANSIStyler("%{level}", this.formatters["_levelStyle"][levelMapper][logRecord["level"]]));
		}
	}
	...
	&& this.formatters[formatterName]["timestamp"].hasOwnProperty("pattern")){
		if (this.formatters[formatterName]["timestamp"]["pattern"].toUpperCase() == "ISO"){
			// logRecord["timestamp"] = logRecord["DateObj"].toISOString();
			tempFormat = tempFormat.replace("%{timestamp}", logRecord["DateObj"].toISOString());
		}
		else{
			let timezone = this.formatters[formatterName]["timestamp"]["timezone"];
			// logRecord["timestamp"] = parser.strptime(
				// this.formatters[formatterName]["timestamp"]["pattern"], timezone, logRecord["DateObj"]
			// );
			tempFormat = tempFormat.replace(
				"%{timestamp}", 
				parser.strptime(this.formatters[formatterName]["timestamp"]["pattern"], timezone, logRecord["DateObj"])
			);
		}
	}
	...
	for (let token in this.formatters[formatterName]){
		...
		if (this.formatters[formatterName][token].hasOwnProperty("transformer") && logRecord.hasOwnProperty(token)){
			if (typeof(this.formatters[formatterName][token]["transformer"]) == "function"){
				...
				tempFormat = tempFormat.replace("%{"+token+"}", this.formatters[formatterName][token].transformer(logRecord[token]));
			}
				...
				tempFormat = tempFormat.replace("%{"+token+"}", this.transformers[transformerName].call(this, logRecord[token]));
			}
		}
	...
	if(this.formatters[formatterName]["json"] == true){
		...
		//Create text output
		logRecord["output"] = parser.parsePlaceholders(tempFormat, logRecord, defaultSubstitution);
	}
