...
function filterLevel(logRecord, context){
	
	if (logRecord["_isHTTPRecord"]){
		//Do not filter HTTP request since the severity cannot be determined
		if (!logRecord.hasOwnProperty("levelNum")){
			return true
		}
		
		//
		if ((context.logOnResponse == false && logRecord.hasOwnProperty("statusCode"))
		|| (context.logOnRequest == false && !logRecord.hasOwnProperty("statusCode"))){
			return false;
		}
	}
...
function runFilters(logRecord, filters){
	for (let i = 0; i < filters.length; ++i){
		if(!filters[i].filter(logRecord)){
			return false;
		}
	}
	return true;
