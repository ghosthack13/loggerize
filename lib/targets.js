...
function getTarget(handleName){
	...
	if (typeof(handleName) == "undefined"){
		...
		else if(this.handles.hasOwnProperty("anonymous") && this.handles["anonymous"].active){
			return this.handles["anonymous"].target;
		}
	}
...
function addTarget(name, func){
	...
	if (typeof(name) == "string" && typeof(func) == "function"){
		if (predefinedTargets.indexOf(name) != -1){
	...
	if (typeof(name) == "object"){
		...
		if (typeof(targetObj["name"]) != "string"){
			let err = new Error("Target object must have name property of type 'string'");
			throw err;
		}
		if (typeof(targetObj["target"]) != "function"){
			let err = new Error("Target must be of type 'function'. Received type: '" + typeof(targetObj["target"]) + "'");
			throw err;
		}
...
function removeTarget(targetName){
	...
	for (let i = 0; i < targetName.length; ++i){
		...
		//Validate that target is not program defined
		if (predefinedTargets.indexOf(targetName[i]) != -1){
